...
class ClientAddressCreateContent extends StatelessWidget {
      ...
      child: FloatingActionButton(
        onPressed: () {
          if (state.formKey!.currentState!.validate()) {
            bloc?.add(FormSubmitClientAddressCreate());
          }
        },
  ...
  Widget _textNewCategory() {
    return Container(
      ...
      child: Text('NUEVA DIRECCION', style: TextStyle(fontSize: 17)),
    );
  }
  ...
  Widget _textFieldAddress() {
    return DefaultTextField(
      ...
      onChanged: (text) {
        bloc?.add(
          AddressChangedClientAddressCreate(address: BlocFormItem(value: text)),
        );
      },
      validator: (value) {
  ...
  Widget _textFieldNeighborhood() {
    return DefaultTextField(
      ...
      onChanged: (text) {
        bloc?.add(
          NeighborhoodChangedClientAddressCreate(
            neighborhood: BlocFormItem(value: text),
          ),
        );
      },
      validator: (value) {
